<?php
    ...
    use Coco\xunsearch\components\XSCommand;
...
class XSSearch extends XSServer
{
    ...
    private $_curDb, $_curDbs                   = [];
    private $_lastDb, $_lastDbs                 = [];
    private $_facets                            = [];
     ...
     * @see XSServer::open
     */
    public function open($conn): static
    {
        parent::open($conn);
        $this->_prefix    = [];
     ...
     * @return XSSearch 返回对象本身以支持串接操作
     */
    public function setFuzzy($value = true): static
    {
     ...
     * @see setFuzzy
     */
    public function setCutOff($percent, $weight = 0): static
    {
     ...
     * @since 1.4.8
     */
    public function setRequireMatchedTerm($value = true): static
    {
     ...
     * @since 1.4.11
     */
    public function setWeightingScheme($scheme): static
    {
     ...
     * @since 1.3.0
     */
    public function setAutoSynonyms($value = true): static
    {
     ...
     * @since  1.4.7
     */
    public function setSynonymScale($value): static
    {
     ...
     * @since 1.3.0
     */
    public function getAllSynonyms($limit = 0, $offset = 0, $stemmed = false): array
    {
        $page        = $limit > 0 ? pack('II', intval($offset), intval($limit)) : '';
        $cmd         = [
            'cmd'  => XS_CMD_SEARCH_GET_SYNONYMS,
            'buf1' => $page,
        ];
     ...
     * @since 1.4.9
     */
    public function getSynonyms($term): bool|array
    {
        $term = strval($term);
        if (strlen($term) === 0) {
            return false;
     ...
     */
    public function getQuery($query = null): mixed
    {
        ...
        if (strpos($res->buf, 'VALUE_RANGE') !== false) {
            ...
            $res->buf = preg_replace_callback($regex, [
                $this,
                'formatValueRange',
            ], $res->buf);
        }
     ...
     * @since 1.4.10
     */
    public function setGeodistSort($fields, $reverse = false, $relevance_first = false): static
    {
     ...
     * @since 1.1.0
     */
    public function setMultiSort($fields, $reverse = false, $relevance_first = false): static
    {
        if (!is_array($fields)) {
     ...
     * @return XSSearch 返回对象本身以支持串接操作
     */
    public function setSort($field, $asc = false, $relevance_first = false): static
    {
        if (is_array($field)) {
     ...
     * @return XSSearch 返回对象本身以支持串接操作
     */
    public function setDocOrder($asc = false): static
    {
     ...
     * @return XSSearch 返回对象本身以支持串接操作
     */
    public function setCollapse($field, $num = 1): static
    {
     ...
     * @return XSSearch 返回对象本身以支持串接操作
     */
    public function addRange($field, $from, $to): static
    {
        if ($from === '' || $from === false) {
            $from = null;
        }
     ...
     * @see addQueryTerm
     */
    public function addWeight($field, $term, $weight = 1): static
    {
     ...
     * @since 1.1.0
     */
    public function setFacets($field, $exact = false): static
    {
        $buf = '';
        if (!is_array($field)) {
            $field = [$field];
     ...
     * @since 1.1.0
     */
    public function getFacets($field = null): array
    {
        if ($field === null) {
            return $this->_facets;
        }

        return $this->_facets[$field] ?? [];
    }
     ...
     * @since 1.4.7
     */
    public function setScwsMulti($level): static
    {
        $level = intval($level);
        if ($level >= 0 && $level < 16) {
            $cmd = [
                'cmd'  => XS_CMD_SEARCH_SCWS_SET,
                'arg1' => XS_CMD_SCWS_SET_MULTI,
                'arg2' => $level,
            ];
            $this->execCommand($cmd);
        }
     ...
     * @return XSSearch 返回对象本身以支持串接操作
     */
    public function setLimit($limit, $offset = 0): static
    {
     ...
     * @throws XSException
     */
    public function setDb($name): static
    {
        $name = strval($name);
        $this->execCommand([
            'cmd' => XS_CMD_SEARCH_SET_DB,
            'buf' => strval($name),
        ]);
     ...
     * @see setDb
     */
    public function addDb($name): static
    {
        $name = strval($name);
        $this->execCommand([
            'cmd' => XS_CMD_SEARCH_ADD_DB,
            'buf' => $name,
        ]);
        $this->_curDbs[] = $name;
     ...
     * @see XS::setScheme
     */
    public function markResetScheme(): static
    {
     ...
     * @throws XSException
     */
    public function terms($query = null, $convert = true): array
    {
        $query = $query === null ? '' : $this->preQueryString($query);
     ...
     */
    public function search($query = null, $saveHighlight = true): array
    {
     ...
     * @return int 数据总量
     */
    public function getDbTotal(): int
    {
     ...
     * @throws XSException
     */
    public function getHotQuery($limit = 6, $type = 'total'): array
    {
        $ret   = [];
     ...
     * @throws XSException
     */
    public function getRelatedQuery($query = null, $limit = 6): array
    {
        $ret   = [];
     ...
     * @return array 返回搜索词组成的数组
     */
    public function getExpandedQuery($query, $limit = 10): array
    {
        $ret   = [];
        $limit = max(1, min(20, intval($limit)));
     ...
     * @return array 返回搜索词组成的数组
     */
    public function getCorrectedQuery($query = null): array
    {
        $ret = [];
        ...
        try {
            ...
            $buf = XS::convert($query, 'UTF-8', $this->_charset);
            $cmd = [
                'cmd' => XS_CMD_QUERY_GET_CORRECTED,
                'buf' => $buf,
            ];
     ...
     * @since 1.1.1
     */
    public function addSearchLog($query, $wdf = 1): static
    {
        $cmd = [
            'cmd' => XS_CMD_SEARCH_ADD_LOG,
            'buf' => $query,
        ];
        if ($wdf > 1) {
            $cmd['buf1'] = pack('i', $wdf);
     ...
     */
    public function highlight($value, $strtr = false): string
    {
        ...
        if (isset($this->_highlight['pairs'])) {
            $value = $strtr ? strtr($value, $this->_highlight['pairs']) : str_replace(array_keys($this->_highlight['pairs']), array_values($this->_highlight['pairs']), $value);
        }
     ...
     */
    public function addQueryString($query, $addOp = XS_CMD_QUERY_OP_AND, $scale = 1): string
    {
     ...
     */
    public function addQueryTerm($field, $term, $addOp = XS_CMD_QUERY_OP_AND, $scale = 1): static
    {
     ...
     */
    private function restoreDb(): static
    {
        $db  = $this->_lastDb;
        $dbs = $this->_lastDbs;
        $this->setDb($db);
     ...
     */
    private function preQueryString($query): string
    {
     ...
     */
    private function regQueryPrefix($name): static
    {
     ...
     * 设置字符型字段及裁剪长度
     */
    private function initSpecialField(): static
    {
        if ($this->_fieldSet === true) {
            return $this;
        }
        foreach ($this->xs->getAllFields() as $field) {
            /* @var $field XSFieldMeta */
            if ($field->cutlen != 0) {
     ...
     */
    private function cleanFieldQuery($query): string
    {
        $query = strtr($query, [
            ' AND ' => ' ',
            ' OR '  => ' ',
     ...
     */
    private function cleanFieldCallback($match): string
    {
     ...
     */
    private function logQuery($query = null): static
    {
        if ($query !== '' && $query !== null) {
            ...
            $query = $this->_query;
            if (!$this->_lastCount || ($this->_defaultOp == XS_CMD_QUERY_OP_OR && strpos($query, ' ')) || strpos($query, ' OR ') || strpos($query, ' NOT ') || strpos($query, ' XOR ')) {
                return $this;
            }
     ...
     * @throws XSException
     */
    private function clearQuery(): static
    {
     ...
     * 初始始化高亮替换数据
     */
    private function initHighlight(): static
    {
        $terms = [];
     ...
     * @return string
     */
    private function formatValueRange($match): string
    {
    ...
    private function numfromstr($str, $index): int
    {
     ...
     */
    private function xapianUnserialise($value): float
    {
        if ($value === "\x80") {
            return 0.0;
        }
