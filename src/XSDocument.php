...
class XSDocument implements \ArrayAccess, \IteratorAggregate
{
     ...
     */
    public function setFields($data): static
    {
        if ($data === null) {
            $this->_data = [];
     ...
     */
    public function setField($name, $value, $isMeta = false): static
    {
        if ($value === null) {
            if ($isMeta) {
     ...
     *
     * @return array|null 索引词列表(词为键, 词重为值), 若无则返回 null
     * @throws XSException
     */
    public function getAddTerms($field): ?array
    {
        $field = strval($field);
     ...
     * @param string $field 字段名称
     *
     * @return string|null 文本内容, 若无则返回 null
     */
    public function getAddIndex($field): ?string
    {
        $field = strval($field);
     ...
     */
    public function addTerm($field, $term, $weight = 1): static
    {
        $field = strval($field);
     ...
     */
    public function addIndex($field, $text): static
    {
        $field = strval($field);
     ...
     */
    public function beforeSubmit(XSIndex $index): bool
    {
     ...
     *
     * @return string|int 转好的字符串
     * @throws XSException
     * @see setCharset
     */
    private function autoConvert($value): string|int
    {
