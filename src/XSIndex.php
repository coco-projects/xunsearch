...
class XSIndex extends XSServer
{
     ...
     */
    public function execCommand($cmd, $res_arg = XS_CMD_NONE, $res_cmd = XS_CMD_OK): mixed
    {
     ...
     * @see update
     */
    public function add(XSDocument $doc): static
    {
     ...
     * @return XSIndex 返回自身对象以支持串接操作
     */
    public function update(XSDocument $doc, $add = false): static
    {
        // before submit
     ...
     * @return XSIndex 返回自身对象以支持串接操作
     */
    public function del($term, $field = null): static
    {
        // get field
        ...
        // get commands
        $cmds  = [];
        $terms = is_array($term) ? array_unique($term) : [$term];
     ...
     * @throw XSException 出错时抛出异常
     */
    public function addExdata($data, $check_file = true): static
    {
        ...
        if ($first != XS_CMD_IMPORT_HEADER && $first != XS_CMD_INDEX_REQUEST && $first != XS_CMD_INDEX_SYNONYMS && $first != XS_CMD_INDEX_REMOVE && $first != XS_CMD_INDEX_EXDATA) {
     ...
     * @since 1.3.0
     */
    public function addSynonym($raw, $synonym): static
    {
        $raw     = strval($raw);
     ...
     * @since 1.3.0
     */
    public function delSynonym($raw, $synonym = null): static
    {
        $raw     = strval($raw);
     ...
     * @since 1.4.7
     */
    public function getScwsMulti(): int
    {
        $cmd = [
            'cmd'  => XS_CMD_SEARCH_SCWS_GET,
            'arg1' => XS_CMD_SCWS_GET_MULTI,
        ];
     ...
     * @return XSIndex 返回自身对象以支持串接操作
     */
    public function openBuffer($size = 4): static
    {
        if ($this->_buf !== '') {
     ...
     * @see openBuffer
     */
    public function closeBuffer(): XSIndex|static
    {
     ...
     * @see endRebuild
     */
    public function beginRebuild(): static
    {
        $this->execCommand([
            'cmd'  => XS_CMD_INDEX_REBUILD,
            'arg1' => 0,
        ], XS_CMD_OK_DB_REBUILD);
        $this->_rebuild = true;
     ...
     * @see beginRebuild
     */
    public function endRebuild(): static
    {
     ...
     * @since 1.3.4
     */
    public function stopRebuild(): static
    {
        try {
            $this->execCommand([
                'cmd'  => XS_CMD_INDEX_REBUILD,
    ...
    {
        $this->execCommand([
            'cmd' => XS_CMD_INDEX_SET_DB,
            'buf' => $name,
        ], XS_CMD_OK_DB_CHANGED);
     ...
     * @throws XSException
     */
    public function flushLogging(): bool
    {
        try {
     ...
     * @return bool 刷新成功返回 true, 失败则返回 false
     */
    public function flushIndex(): bool
    {
        try {
     ...
     * @throw XSException 出错时抛出异常
     */
    public function setCustomDict($content): static
    {
        $cmd = [
            'cmd'  => XS_CMD_INDEX_USER_DICT,
            'arg1' => 1,
            'buf'  => $content,
        ];
     ...
     * 关闭索引服务端连接
     */
    public function close($ioerr = false): static
    {
        $this->closeBuffer();
        parent::close($ioerr);
     ...
     */
    private function appendBuffer($buf): static
    {
        $this->_buf .= $buf;
