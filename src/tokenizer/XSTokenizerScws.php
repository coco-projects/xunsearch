...
class XSTokenizerScws implements XSTokenizer
{
     ...
     * @return XSTokenizerScws 返回对象本身以支持串接操作
     */
    public function setIgnore($yes = true): static
    {
        $this->_setting['ignore'] = new XSCommand(XS_CMD_SEARCH_SCWS_SET, XS_CMD_SCWS_SET_IGNORE, $yes === false ? 0 : 1);
     ...
     * @return XSTokenizerScws 返回对象本身以支持串接操作
     */
    public function setMulti($mode = 3): static
    {
     ...
     * @return XSTokenizerScws 返回对象本身以支持串接操作
     */
    public function setDict($fpath, $mode = null): static
    {
        if (!is_int($mode)) {
     ...
     * @return XSTokenizerScws 返回对象本身以支持串接操作
     */
    public function addDict($fpath, $mode = null): static
    {
        if (!is_int($mode)) {
            $mode = stripos($fpath, '.txt') !== false ? SCWS_XDICT_TXT : SCWS_XDICT_XDB;
        }
        if (!isset($this->_setting['add_dict'])) {
            $this->_setting['add_dict'] = [];
        }
     ...
     * @return XSTokenizerScws 返回对象本身以支持串接操作
     */
    public function setDuality($yes = true): static
    {
        $this->_setting['duality'] = new XSCommand(XS_CMD_SEARCH_SCWS_SET, XS_CMD_SCWS_SET_DUALITY, $yes === false ? 0 : 1);
     ...
     */
    public function getResult($text): array
    {
        $words = [];
     ...
     */
    public function getTops($text, $limit = 10, $xattr = ''): array
    {
        $words = [];
        $text  = $this->applySetting($text);
     ...
     */
    public function hasWord($text, $xattr): bool
    {
        $text = $this->applySetting($text);
